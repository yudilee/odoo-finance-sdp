...
<div x-data="{
    ...
    async doQuickSync() {
        ...
        try {
            const res = await fetch('{{ route('invoice-proforma.sync-recent', [], false) }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content,
        ...
        <div x-show="filtersOpen" x-cloak x-transition class="p-4 border-t border-slate-200 dark:border-slate-700">
            ...
            <div class="flex flex-wrap items-end gap-3 mb-3">
                ...
                <div class="flex gap-2 items-end pb-[2px]">
                    ...
                    <a href="{{ route('invoice-proforma.index') }}" class="px-3 py-2 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-sm font-medium rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">Clear</a>
                    ...
                    <button type="button" class="printSelectedHubBtn px-3 py-2 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700 transition-colors flex items-center gap-1 opacity-50 cursor-not-allowed" disabled data-doc-type="invoice_proforma">
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    if (value === 'detail_username') {
                        htmlUrl += '?print_mode=detail&show_username=1';
                        pdfUrl += '?print_mode=detail&show_username=1';
