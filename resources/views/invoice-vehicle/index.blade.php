...
<div x-data="{
    ...
    async doSync() {
        ...
        try {
            // Phase 1: Get IDs
            const idRes = await fetch('{{ route('invoice-vehicle.sync-ids', [], false) }}', {
                method: 'POST',
                headers: {
            ...
            for (let i = 0; i < allIds.length; i += chunkSize) {
                ...
                const batchRes = await fetch('{{ route('invoice-vehicle.sync-batch', [], false) }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content,
    ...
    async doQuickSync() {
        ...
        try {
            const res = await fetch('{{ route('invoice-vehicle.sync-recent', [], false) }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content,
        ...
        <div x-show="filtersOpen" x-cloak x-transition class="p-4 border-t border-slate-200 dark:border-slate-700">
            ...
            <div class="flex flex-wrap items-end gap-3 mb-3">
                ...
                <div class="flex gap-2 items-end pb-[2px]">
                    ...
                    <a href="{{ route('invoice-vehicle.index') }}" class="px-3 py-2 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-sm font-medium rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">Clear</a>
                    ...
                    <button type="button" class="printSelectedHubBtn px-3 py-2 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700 transition-colors flex items-center gap-1 opacity-50 cursor-not-allowed" disabled data-doc-type="invoice_vehicle">
                    ...
                    <tr class="border-t border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                        ...
                        <td x-show="columns.name.visible" class="px-3 py-2 font-mono text-xs font-semibold whitespace-nowrap text-blue-600 dark:text-blue-400">
                            <div class="flex items-center gap-2">
                                ...
                                <button type="button" onclick="printInvoiceToHub('{{ $invoice->name }}', 'invoice_vehicle')" title="Print to Hub" class="text-slate-400 hover:text-emerald-600 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                                </button>
                                <button type="button" onclick="printInvoice('{{ $invoice->name }}', '{{ route('invoice-vehicle.print', $invoice) }}')" title="Print PDF" class="text-slate-400 hover:text-indigo-600 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                </button>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (bulkForm) {
            bulkForm.addEventListener('submit', function(e) {
                ...
                const baseActionUrl = isHtml ? "{{ route('invoice-vehicle.print-selected-html') }}" : "{{ route('invoice-vehicle.print-selected') }}";
        ...
        hubBtns.forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                const docType = btn.dataset.docType || 'invoice_vehicle';
