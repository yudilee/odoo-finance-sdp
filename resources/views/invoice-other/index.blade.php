...
<div x-data="{
    ...
    init() {
        const saved = localStorage.getItem('invother_column_settings');
        if (saved) {
        ...
        this.$watch('columns', value => {
            localStorage.setItem('invother_column_settings', JSON.stringify(value));
        }, { deep: true });
    },
    resize(key, event) {
        ...
        <div class="px-4 py-3 bg-slate-50/50 dark:bg-slate-900/50 flex items-center justify-between cursor-pointer group" @click="filtersOpen = !filtersOpen">
            <div class="flex items-center gap-4 text-sm">
                ...
                <div class="hidden md:flex items-center gap-2 text-xs text-slate-400">
                    @if(request('type')) <span class="px-1.5 py-0.5 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded">{{ request('type') === 'with_tax' ? 'With Tax' : 'Without Tax' }}</span> @endif
        ...
        <div x-show="filtersOpen" x-cloak x-transition class="p-4 border-t border-slate-200 dark:border-slate-700">
            ...
            <div class="flex flex-wrap items-end gap-3 mb-3">
                ...
                <div>
                    ...
                    <select name="type" class="px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500">
                        <option value="">All Types</option>
                        <option value="with_tax" {{ request('type') === 'with_tax' ? 'selected' : '' }}>With Tax (INVOT)</option>
                        <option value="without_tax" {{ request('type') === 'without_tax' ? 'selected' : '' }}>Without Tax (INVOW)</option>
                    </select>
                </div>
                <div>
                ...
                <div class="flex gap-2 items-end pb-[2px]">
                    ...
                    <a href="{{ route('invoice-other.index') }}" class="px-4 py-2 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-sm font-medium rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">Clear</a>
            ...
            <div x-show="syncOpen" x-cloak x-transition class="mt-3 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl">
                <h3 class="text-sm font-semibold text-blue-700 dark:text-blue-300 mb-3">Sync Invoice Other from Odoo</h3>
                ...
                <tbody>
                    ...
                    <tr class="border-t border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <div x-data="{ open: false, printContract: true }" @click.away="open = false" class="relative" style="display:inline-block;">
                                    ...
                                    <div x-show="open" x-cloak class="absolute left-0 mt-1 w-48 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl z-50 py-1" style="top: 100%;">
                                        ...
                                        <a :href="'{{ route('invoice-other.kuitansi-pdf', $invoice) }}' + (printContract ? '?show_contract=1' : '?show_contract=0')" target="_blank" class="flex items-center gap-2 px-3 py-1.5 text-xs text-slate-700 dark:text-slate-300 hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:text-amber-700">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                            Download PDF
                                        </a>
                                        <a :href="'{{ route('invoice-other.kuitansi-html', $invoice) }}' + (printContract ? '?show_contract=1' : '?show_contract=0')" target="_blank" class="flex items-center gap-2 px-3 py-1.5 text-xs text-slate-700 dark:text-slate-300 hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:text-amber-700">
                        ...
                        <td x-show="columns.journal.visible" class="px-3 py-2 text-xs">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium {{ str_starts_with($invoice->name, 'INVOT') ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : 'bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400' }}">
                    ...
                    <tr>
                        <td :colspan="visibleColumnCount" class="px-4 py-12 text-center">
                            <div class="text-slate-400">
                                ...
                                <p class="text-lg font-medium">No invoice other entries found</p>
