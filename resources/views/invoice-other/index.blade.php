...
<div x-data="{
    ...
    async doSync() {
        ...
        try {
            // Phase 1: Get IDs
            const idRes = await fetch('{{ route('invoice-other.sync-ids', [], false) }}', {
                method: 'POST',
                headers: {
            ...
            for (let i = 0; i < allIds.length; i += chunkSize) {
                ...
                const batchRes = await fetch('{{ route('invoice-other.sync-batch', [], false) }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content,
    ...
    async doQuickSync() {
        ...
        try {
            const res = await fetch('{{ route('invoice-other.sync-recent', [], false) }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content,
        ...
        <div x-show="filtersOpen" x-cloak x-transition class="p-4 border-t border-slate-200 dark:border-slate-700">
            ...
            <div class="flex flex-wrap items-end gap-3 mb-3">
                ...
                <div class="flex gap-2 items-end pb-[2px]">
                    ...
                    <a href="{{ route('invoice-other.index') }}" class="px-3 py-2 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-sm font-medium rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">Clear</a>
                    ...
                    <button type="button" class="printSelectedHubBtn px-3 py-2 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700 transition-colors flex items-center gap-1 opacity-50 cursor-not-allowed" disabled data-doc-type="invoice_other">
