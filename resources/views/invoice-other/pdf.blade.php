...
<body>
    ...
    @php
        ...
        if (!isset($printMode) || $printMode !== 'summary') {
            $otherSubtotal = $otherSubtotal - $discountTotal - $roundingLines->sum(fn($l) => $l->quantity * $l->price_unit ?: $l->price_unit);
        }
                    ...
                    <td colspan="{{ $showUnitColumn ? 5 : 4 }}" style="border: none; padding: 0 0 10px 0; background-color: white; position: relative;">
                        ...
                        <table class="company-header" style="border-spacing: 0;">
                            <tr>
                                ...
                                <td style="width: 40%;">
                                    <div class="invoice-title">{{ str_starts_with($invoice->name, 'RINVOT') ? 'CREDIT NOTES' : 'INVOICE' }}</div>
