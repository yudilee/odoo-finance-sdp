...
@section('subtitle', 'Invoice Other Detail')
...
<div class="max-w-5xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <a href="{{ route('invoice-other.index') }}" class="flex items-center gap-2 text-sm text-slate-500 hover:text-emerald-500 transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            Back to list
        </a>
        <div class="flex items-center gap-2">
            @if($prev)
            <a href="{{ route('invoice-other.show', $prev) }}" class="px-3 py-1.5 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-medium rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">← Newer</a>
            @endif
            @if($next)
            <a href="{{ route('invoice-other.show', $next) }}" class="px-3 py-1.5 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-medium rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">Older →</a>
            @endif
            <button type="button"
                    onclick="printInvoiceToHub('{{ $invoice->name }}', 'invoice_other')"
                    class="px-3 py-1.5 bg-emerald-600 text-white text-xs font-medium rounded-lg hover:bg-emerald-700 transition-colors flex items-center gap-1">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                Print to Hub
            </button>
            <button type="button" onclick="showPrintOptions()" class="px-3 py-1.5 bg-indigo-600 text-white text-xs font-medium rounded-lg hover:bg-indigo-700 transition-colors flex items-center gap-1">
...
function showPrintOptions() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let baseUrl = "{{ route('invoice-other.print', $invoice) }}";
            ...
            <table class="w-full text-sm">
                <thead class="bg-slate-50 dark:bg-slate-900">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-center text-xs font-medium text-slate-500 w-20">Qty</th>
