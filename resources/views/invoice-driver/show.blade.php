...
@section('subtitle', 'Invoice Detail')

@section('content')
<div class="max-w-5xl mx-auto">
    {{-- Navigation --}}
    <div class="flex items-center justify-between mb-6">
        <a href="{{ route('invoice-driver.index') }}" class="flex items-center gap-2 text-sm text-slate-500 hover:text-emerald-500 transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            Back to list
        </a>
        <div class="flex items-center gap-2">
            @if($prev)
            <a href="{{ route('invoice-driver.show', $prev) }}" class="px-3 py-1.5 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-medium rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">← Newer</a>
            @endif
            @if($next)
            <a href="{{ route('invoice-driver.show', $next) }}" class="px-3 py-1.5 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-medium rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">Older →</a>
            @endif
            <button type="button"
                    onclick="printInvoiceToHub('{{ $invoice->name }}', 'invoice_driver')"
            ...
            <button type="button" onclick="printInvoice('{{ $invoice->name }}', '{{ route('invoice-driver.print', $invoice) }}')" class="px-3 py-1.5 bg-indigo-600 text-white text-xs font-medium rounded-lg hover:bg-indigo-700 transition-colors flex items-center gap-1">
