        ...
        <div class="invoice-page" style="{{ $loop->last ? 'page-break-after: auto;' : 'page-break-after: always;' }}">
            ...
            <table class="lines-table">
                <thead>
                    <tr>
                        <td colspan="5"
                            style="border: none; padding: 0 0 10px 0; background-color: white; position: relative;">
                            ...
                            <table class="company-header" style="border-spacing: 0;">
                                <tr>
                                    <td style="width: 60%; padding: 0;">
                                        ...
                                        @endphp
                                        <img src="{{ $logoSource }}"
                                            style="max-height: 45px; max-width: 180px; margin-bottom: 5px;" alt="Logo"><br>
                                    ...
                                    <td style="width: 40%;">
                                        <div class="invoice-title">{{ str_starts_with($invoice->name, 'RINVDV') ? 'CREDIT NOTES' : 'INVOICE' }}</div>
                                            ...
                                            <tr>
                                                <td colspan="3" style="padding-bottom: 5px; padding-left: 0;">
                                                    ...
                                                    @if($invoice->partner_npwp)
                                                        <div
                                                            style="font-size: 9px; margin-top: 3px; font-weight: bold; padding-top: 5px;">
                                                            NPWP : {{ $invoice->partner_npwp }}</div>
                                                    @endif
                                                </td>
                                            </tr>
                                        </table>
                ...
                <tbody>
                    @php
                        ...
                        $managerName = !empty($invoice->manager_name)
                            ...
                            : ($defaultManager ?? \App\Models\Setting::get('default_bc_manager', ''));
                        $spvName = !empty($invoice->spv_name)
                            ? $invoice->spv_name
                            : ($defaultSpv ?? \App\Models\Setting::get('default_bc_spv', ''));

                        $displayLines = collect();
                        if (isset($printMode) && $printMode === 'summary') {
                            // Summary Mode: Use note lines as description, or narration, or fallback
                            if ($noteLines->isNotEmpty()) {
                                ...
                                // Fallback to combining regular line descriptions
