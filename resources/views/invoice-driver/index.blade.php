...
<div x-data="{
    ...
    <div x-data="{ filtersOpen: true }" class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 mb-6 overflow-hidden">
        ...
        <div x-show="filtersOpen" x-cloak x-transition class="p-4 border-t border-slate-200 dark:border-slate-700">
            <form method="GET" action="{{ route('invoice-driver.index', [], false) }}">
            <div class="flex flex-wrap items-end gap-3 mb-3">
                ...
                <div class="flex gap-2 items-end pb-[2px]">
                    ...
                    <a href="{{ route('invoice-driver.index') }}" class="px-4 py-2 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-sm font-medium rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">Clear</a>
    ...
    {{-- Table --}}
    <form id="bulkPrintForm" method="POST" action="{{ route('invoice-driver.print-selected', [], false) }}" target="_blank">
        @csrf
                ...
                <tbody>
                    ...
                    <tr class="border-t border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                        ...
                        <td x-show="columns.name.visible" class="px-3 py-2 font-mono text-xs font-semibold text-emerald-600 dark:text-emerald-400 whitespace-nowrap">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('invoice-driver.show', $invoice) }}" class="hover:underline">{{ $invoice->name }}</a>
                                <a href="{{ route('invoice-driver.print-html', $invoice) }}" target="_blank" title="Print Browser" class="text-slate-400 hover:text-emerald-600 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                                </a>
                                ...
                                <div x-data="{ open: false, printContract: true }" @click.away="open = false" class="relative" style="display:inline-block;">
                                    ...
                                    <div x-show="open" x-cloak class="absolute left-0 mt-1 w-48 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl z-50 py-1" style="top: 100%;">
                                        ...
                                        <a :href="'{{ route('invoice-driver.kuitansi-pdf', $invoice) }}' + (printContract ? '?show_contract=1' : '?show_contract=0')" target="_blank" class="flex items-center gap-2 px-3 py-1.5 text-xs text-slate-700 dark:text-slate-300 hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:text-amber-700">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                            Download PDF
                                        </a>
                                        <a :href="'{{ route('invoice-driver.kuitansi-html', $invoice) }}' + (printContract ? '?show_contract=1' : '?show_contract=0')" target="_blank" class="flex items-center gap-2 px-3 py-1.5 text-xs text-slate-700 dark:text-slate-300 hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:text-amber-700">
