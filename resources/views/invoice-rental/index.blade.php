...
<div x-data="{
    ...
    <div x-data="{ filtersOpen: true }" class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 mb-6 overflow-hidden">
        ...
        <div x-show="filtersOpen" x-cloak x-transition class="p-4 border-t border-slate-200 dark:border-slate-700">
            <form method="GET" action="{{ route('invoice-rental.index') }}">
    ...
    {{-- Table --}}
    <form id="bulkPrintForm" method="POST" action="{{ route('invoice-rental.print-selected') }}" target="_blank">
        @csrf
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateSelection() {
            ...
            if (printBtn) {
                ...
                printBtn.classList.toggle('opacity-50', checkedCount === 0);
                printBtn.classList.toggle('cursor-not-allowed', checkedCount === 0);
            }
        }
            ...
            bulkForm.addEventListener('submit', function(e) {
                ...
                if (hasInvrs) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            const value = result.value;
                            let actionUrl = "{{ route('invoice-rental.print-selected') }}";
                            if (value === 'detail_username') {
